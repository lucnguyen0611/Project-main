...
export default function StudentExamDetail() {
    ...
    useEffect(() => {
        ...
        const onPopState = () => {
            if (submittedRef.current) return;
            const confirmLeave = window.confirm("Bạn chưa nộp bài. Thoát sẽ mất dữ liệu. Tiếp tục?");
            if (!confirmLeave) {
                window.history.pushState(null, "", window.location.href);
            }
        };
    ...
    const submitExam = useCallback(
        async (status: "doing" | "completed") => {
            ...
            try {
                ...
                await examResultApi.createExamResult(payload); // <— bạn có thể dùng PUT hoặc POST tùy API
                showToast?.("Nộp bài thành công!", "success");
